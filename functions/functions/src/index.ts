    ...
    try {
      ...
      if (aiResponse.moodPrediction > entry.moodBefore) {
        await snap.ref.update({
          ...
          aiProcessed: true,
          aiProcessingCompleted: FieldValue.serverTimestamp()
        });
      }
...
export const crisisPrevention = functions
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const interventionsTriggered: Array<{
        userId: string;
        riskLevel: any;
...
export const upgradeUserTier = functions
  .runWith({ memory: '256MB' })
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      return {
        success: true,
        subscriptionId: subscription.id,
        clientSecret: typeof (subscription.latest_invoice as Stripe.Invoice)?.payment_intent === 'object' 
...
async function buildPersonalContext(userId: string): Promise<PersonalContext> {
  try {
...
function getProcessingPriority(tier: string): 'high' | 'standard' | 'low' {
  switch (tier) {
...
async function processAIReflection(
...
function buildTraumaInformedPrompt(request: AIProcessingRequest): string {
...
async function getActiveUsers(): Promise<Array<{id: string}>> {
...
async function analyzeRiskPatterns(userId: string): Promise<any> {
  return { interventionNeeded: false, riskLevel: 'low', interventionType: 'none' };
}

async function triggerGentleIntervention(userId: string, assessment: any): Promise<void> {
  // Implementation would send appropriate intervention
}

async function storeCrisisPreventionAnalytics(interventions: any[]): Promise<void> {
  // Implementation would store analytics
}

async function storeAIInsight(userId: string, entryId: string, aiResponse: any, tier: string): Promise<void> {
  const insight = {
    userId,
    entryId,
...
async function storeEmergencyReflection(userId: string, entryId: string, text: string, mood: number): Promise<void> {
...
function generateTraumaInformedReflection(request: AIProcessingRequest): string {
  ...
  const style = personalContext.communicationStyle;
  const styleReflections = (style && reflections[style as keyof typeof reflections]) || reflections.encouraging;
  const selectedReflection = styleReflections[Math.floor(Math.random() * styleReflections.length)];
  return selectedReflection || 'Thank you for sharing your thoughts with such openness and courage.';
}

function generatePersonalizedActions(request: AIProcessingRequest): string[] {
  const baseActions = [
...
function assessRiskLevel(request: AIProcessingRequest): 'low' | 'moderate' | 'high' {
...
function detectCelebrationMoment(request: AIProcessingRequest): boolean {
  const text = request.text.toLowerCase();
...
async function isValidUpgrade(currentTier: string, targetTier: string): Promise<boolean> {
  ...
  const validPaths = upgradePaths[currentTier];
  return validPaths ? validPaths.includes(targetTier) : false;
}
...
async function updateUserTier(userId: string, tierData: UserTier): Promise<void> {
  // Update user tier in appropriate collection
}

async function sendTierWelcomeSequence(userId: string, tier: string): Promise<void> {
  // Send welcome sequence for new tier
}

async function trackTierConversion(userId: string, fromTier: string, toTier: string): Promise<void> {
  // Track conversion analytics
}

async function checkMilestoneAchievements(userId: string, entry: any, aiResponse: any): Promise<void> {
  // Check for milestone achievements
}

async function updateEngagementMetrics(userId: string, entry: any, aiResponse: any): Promise<void> {
  // Update user engagement metrics
}

async function scheduleCrisisIntervention(userId: string, aiResponse: any): Promise<void> {
  // Schedule crisis intervention
}

// ===== BETA MANAGEMENT SYSTEM =====

export * from './beta-management';
...
export * from './waitlist-automation';
...
const nextjsServer = next({
  dev: isDev,
  dir: __dirname,  // Use the functions directory where .next is now copied
  conf: {
    distDir: '.next'
  }
});
