...
import { CommunityHealingService } from "./communityService";
import { validateSession, checkRateLimit } from "./utils/communityUtils";

const communityService = new CommunityHealingService();

// === ANONYMOUS STORY SHARING ===
export const createCommunityStory = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
    ...
    // Validate session
    const { userId, isValid } = await validateSession(sessionToken);
    if (!isValid) {
      throw new HttpsError("unauthenticated", "Invalid session");
    }

    // Rate limiting - max 3 stories per day
    const canPost = await checkRateLimit(userId, "create_story", 24 * 60, 3);
    if (!canPost) {
      throw new HttpsError("resource-exhausted", "Story posting limit exceeded");
    }

    // Validate story data
    if (!storyData.content || storyData.content.length < 50 || storyData.content.length > 5000) {
      throw new HttpsError("invalid-argument", "Story content must be between 50 and 5000 characters");
    }
    ...
    if (!allowedStages.includes(storyData.healingStage)) {
      throw new HttpsError("invalid-argument", "Invalid healing stage");
    }
    ...
    res.status(error instanceof HttpsError ? 400 : 500).json({
      error: errorMessage
    });
  }
});

export const getStoriesByStage = onRequest(async (req, res) => {
  try {
    if (req.method !== "GET") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    if (!stage) {
      throw new HttpsError("invalid-argument", "Healing stage is required");
    }

    const stories = await communityService.getStoriesByHealingStage(stage as string, Number(limit));
...
export const reactToStory = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
...
// === HEALING CIRCLES ===
export const createHealingCircle = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    if (!userRecord.customClaims?.facilitator) {
      throw new HttpsError("permission-denied", "Insufficient permissions to create healing circles");
    }
...
export const joinHealingCircle = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
...
export const getHealingCircles = onRequest(async (req, res) => {
  try {
    if (req.method !== "GET") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    const circles = await communityService.getActiveHealingCircles(topic as string);
...
// === COMMUNITY WISDOM ===
export const createWisdomEntry = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    const canPost = await checkRateLimit(userId, "create_wisdom", 24 * 60, 5);
    if (!canPost) {
      throw new HttpsError("resource-exhausted", "Wisdom sharing limit exceeded");
    }
...
export const searchWisdom = onRequest(async (req, res) => {
  try {
    if (req.method !== "GET") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    const entries = await communityService.searchWisdomEntries(
      query as string,
      category as string,
      tagsArray
    );
...
export const voteOnWisdom = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    if (!allowedVotes.includes(vote)) {
      throw new HttpsError("invalid-argument", "Invalid vote type");
    }
...
export const createCollectiveExperience = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    const userRecord = await getAuth().getUser(userId);
    if (!userRecord.customClaims?.facilitator) {
      throw new HttpsError("permission-denied", "Insufficient permissions to create collective experiences");
    }
...
export const joinCollectiveExperience = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
...
// === PEER MATCHING ===
export const createPeerProfile = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
...
// === CONTENT REPORTING ===
export const reportContent = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }
    ...
    const canReport = await checkRateLimit(userId, "report_content", 60, 5);
    if (!canReport) {
      throw new HttpsError("resource-exhausted", "Report limit exceeded");
    }
...
// === SCHEDULED FUNCTIONS ===
export const cleanupExpiredContent = onSchedule("0 2 * * *", async () => {
...
export const generateCommunityInsights = onSchedule("0 1 * * 1", async () => {
...
export const moderateContent = onRequest(async (req, res) => {
  try {
    if (req.method !== "POST") {
      throw new HttpsError("invalid-argument", "Method not allowed");
    }

    const { content, contentType }: { content: string; contentType: string } = req.body;
