...
export interface CrisisAssessment {
  ...
  indicators: string[];
  recommendedAction: 'monitor' | 'check_in' | 'immediate_intervention' | 'emergency_response';
  reasoning: string;
...
Content: "${content}"`
    ...
    if (assessment.riskLevel === 'critical' || assessment.riskLevel === 'high') {
      await triggerCrisisIntervention(userId, assessment, assessmentRef.id);
    }
...
const getInterventionType = (recommendedAction: string): string => {
...
const sendCrisisNotification = async (userId: string, assessment: CrisisAssessment) => {
...
const getCrisisMessage = (riskLevel: string): string => {
  switch (riskLevel) {
    case 'critical':
...
const alertCrisisTeam = async (userId: string, assessment: CrisisAssessment, interventionId: string) => {
...
export const analyzeBatchContent = functions.https.onCall(async (data, context) => {
  try {
    const { userId } = data;
...
export const getCrisisResources = functions.https.onCall(async (data, context) => {
  ...
  return {
    resources: resources[riskLevelMap[riskLevel as keyof typeof riskLevelMap]] || resources.monitoring
  };
});