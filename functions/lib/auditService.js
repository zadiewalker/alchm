...
exports.detectAuditAnomalies = exports.generateComplianceAuditReport = exports.getUserAuditTrail = void 0;
exports.logPrivacyAction = logPrivacyAction;
 ...
 */
async function logPrivacyAction(auditData) {
    try {
        const logEntry = {
            ...
            legalBasis: auditData.legalBasis,
 ...
 */
function generateComplianceTags(action, legalBasis) {
    const tags = ['privacy_audit'];
    // Add GDPR-specific tags
    if (legalBasis === 'consent')
        tags.push('gdpr_article_6_1_a');
    if (legalBasis === 'legitimate_interest')
        tags.push('gdpr_article_6_1_f');
    if (legalBasis === 'vital_interests')
        tags.push('gdpr_article_6_1_d');
    if (legalBasis === 'legal_obligation')
        tags.push('gdpr_article_6_1_c');
    // Add action-specific tags
    if (action.includes('data_export'))
        tags.push('gdpr_article_20', 'data_portability');
    if (action.includes('deletion'))
        tags.push('gdpr_article_17', 'right_to_erasure');
    if (action.includes('consent'))
        tags.push('gdpr_article_7', 'consent_management');
    if (action.includes('breach'))
        tags.push('gdpr_article_33', 'breach_notification');
    return tags;
}
/**
 * Create high-risk alerts for immediate attention
 */
async function createHighRiskAlert(auditId, auditData) {
 ...
 */
async function updateUserAuditSummary(userId, action) {
 ...
 */
async function attemptBackupAuditLog(auditData, error) {
    try {
        await db.collection('auditLogBackup').add(Object.assign(Object.assign({}, auditData), { originalError: error.message, backupTimestamp: admin.firestore.FieldValue.serverTimestamp(), requiresManualReview: true }));
    }
    catch (backupError) {
 ...
 */
exports.getUserAuditTrail = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
    ...
    try {
        ...
        return {
            ...
            summary: {
                ...
                actionCounts: (summary === null || summary === void 0 ? void 0 : summary.actionCounts) || {},
                lastActivity: (_a = summary === null || summary === void 0 ? void 0 : summary.lastActivity) === null || _a === void 0 ? void 0 : _a.toDate()
            }
        };
    }
    catch (error) {
 ...
 */
exports.generateComplianceAuditReport = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const complianceReport = {
            ...
            gdprCompliance: {
                consentActions: auditLogs.filter(log => { var _a; return (_a = log.complianceTags) === null || _a === void 0 ? void 0 : _a.includes('consent_management'); }).length,
                dataPortabilityRequests: auditLogs.filter(log => { var _a; return (_a = log.complianceTags) === null || _a === void 0 ? void 0 : _a.includes('data_portability'); }).length,
                erasureRequests: auditLogs.filter(log => { var _a; return (_a = log.complianceTags) === null || _a === void 0 ? void 0 : _a.includes('right_to_erasure'); }).length,
                breachNotifications: auditLogs.filter(log => { var _a; return (_a = log.complianceTags) === null || _a === void 0 ? void 0 : _a.includes('breach_notification'); }).length
            },
 ...
 */
exports.detectAuditAnomalies = functions.pubsub.schedule('0 6 * * *').onRun(async () => {
    try {
        ...
        const consentChanges = auditLogs.filter(log => log.action.includes('consent_') && (log.action.includes('withdrawn') || log.action.includes('updated')));
        const userConsentChanges = {};
 ...
 */
function getActionBreakdown(auditLogs) {
    const breakdown = {};
...
function getLegalBasisAnalysis(auditLogs) {
    const analysis = {};
    auditLogs.forEach(log => {
...
function getRiskAssessment(auditLogs) {
    return {
...
function generateComplianceRecommendations(auditLogs) {
...
async function isAdmin(userId) {
    ...
    try {
        ...
        return adminDoc.exists && ((_a = adminDoc.data()) === null || _a === void 0 ? void 0 : _a.verified) === true;
    }
    catch (error) {
        return false;
    }
