...
exports.CommunityModerationService = void 0;
...
class CommunityModerationService {
    ...
    async moderateContent(content, contentType, userId, contextData) {
        try {
            const flags = [];
            let confidenceScore = 1.0;
            let crisisLevel = undefined;
            ...
            const requiresHumanReview = flags.some(f => f.severity === 'critical' || f.severity === 'high') ||
                confidenceScore < 0.5 ||
                crisisLevel === 'high' || crisisLevel === 'critical';
            // Log moderation decision
            await this.logModerationDecision({
                contentType,
                contentId: (contextData === null || contextData === void 0 ? void 0 : contextData.contentId) || 'unknown',
                userId,
                flags,
                confidenceScore,
                isApproved,
    ...
    async detectCrisisContent(content) {
        var _a;
        ...
        for (const pattern of crisisPatterns) {
            if (pattern.test(contentLower)) {
                return {
                    ...
                    triggerText: (_a = contentLower.match(pattern)) === null || _a === void 0 ? void 0 : _a[0]
                };
            }
        }
        return {
    ...
    async performAIAnalysis(content) {
        try {
            ...
            const foundNegativeWords = negativeWords.filter(word => content.toLowerCase().includes(word));
    ...
    async updateModerationStats(data) {
        try {
    ...
    async triggerCrisisEscalation(userId, contentType, flags, content) {
        try {
            ...
            await (0, communityUtils_1.escalateCrisisAlert)(userId, 'community_content', crisisFlag.severity);
    ...
    async sendImmediateCrisisResources(userId) {
        try {
    ...
    async reviewFlaggedContent(moderatorId, contentId, decision, notes) {
        try {
            ...
            for (const collection of collections) {
                ...
                if (doc.exists) {
                    await doc.ref.update({
                        ...
                        humanReviewedAt: firestore_1.Timestamp.now(),
            ...
            await db.collection('communityModerationLogs').add({
                contentType: 'unknown', // Would be determined from content lookup
                contentId,
                moderatorId,
                action: decision,
                reason: notes,
    ...
    async getCommunityHealthMetrics() {
        ...
        try {
            ...
            const moderationSnapshot = await db.collection('communityModerationLogs')
                .where('timestamp', '>=', firestore_1.Timestamp.fromDate(last7Days))
            ...
            const totalContent = (((_a = storiesSnapshot.data()) === null || _a === void 0 ? void 0 : _a.count) || 0) +
                (((_b = wisdomSnapshot.data()) === null || _b === void 0 ? void 0 : _b.count) || 0) +
                (((_c = circlesSnapshot.data()) === null || _c === void 0 ? void 0 : _c.count) || 0);
            return {
...
exports.CommunityModerationService = CommunityModerationService;
//# sourceMappingURL=communityModerationService.js.map