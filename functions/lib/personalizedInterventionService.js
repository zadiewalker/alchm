...
exports.PersonalizedInterventionService = void 0;
...
class PersonalizedInterventionService {
     ...
     */
    async generatePersonalizedInterventions(userId) {
        try {
            ...
            const scheduledInterventions = await this.generateScheduledInterventions(userProfile, timingModel, interventionConfig);
            const schedule = {
     ...
     */
    async buildCircadianProfile(userId) {
     ...
     */
    async buildReceptivityModel(userProfile) {
        var _a, _b;
        const prompt = `
    ...
    `;
        ...
        try {
            const modelData = JSON.parse(((_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || "{}");
            return {
     ...
     */
    async createOptimalTimingModel(userProfile, receptivityModel) {
        return {
            userId: userProfile.userId,
     ...
     */
    async createAdaptiveConfig(userProfile, timingModel) {
        return {
            userId: userProfile.userId,
            intensitySettings: {
     ...
     */
    async generateScheduledInterventions(userProfile, timingModel, config) {
        const interventions = [];
        ...
        for (let day = 0; day < 14; day++) {
            ...
            if (this.shouldScheduleIntervention('daily_check_in', day, config)) {
                const optimalTime = this.findBestTimeForDate(targetDate, timingModel.interventionTypeTimings['daily_check_in'].optimal, userProfile);
                interventions.push({
            ...
            if (day % 7 === 0 && this.shouldScheduleIntervention('therapeutic_reminder', day, config)) {
                const optimalTime = this.findBestTimeForDate(targetDate, timingModel.interventionTypeTimings['therapeutic_reminder'].optimal, userProfile);
                interventions.push({
                    id: `intervention_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                    userId: userProfile.userId,
     ...
     */
    async storeInterventionSchedule(schedule) {
        await this.db.collection('interventionSchedules').doc(schedule.userId).set(Object.assign(Object.assign({}, schedule), { timestamp: admin.firestore.FieldValue.serverTimestamp() }));
        ...
        schedule.scheduledInterventions.forEach(intervention => {
            ...
            batch.set(docRef, Object.assign(Object.assign({}, intervention), { timestamp: admin.firestore.FieldValue.serverTimestamp(), status: 'scheduled' }));
        });
        await batch.commit();
    }
    // Helper methods for data analysis and calculations
    async getUserJournalingPatterns(userId) {
        ...
        return snapshot.docs.map(doc => {
            ...
            return ({
                ...
                emotionalState: ((_b = doc.data().emotionalThemes) === null || _b === void 0 ? void 0 : _b.primaryEmotions) || []
            });
        });
    }
    async getUserVoiceActivityPatterns(userId) {
        ...
        return snapshot.docs.map(doc => {
            ...
            return ({
                ...
                duration: ((_a = doc.data().transcription) === null || _a === void 0 ? void 0 : _a.duration) || 0,
                emotionalState: ((_b = doc.data().emotionalAnalysis) === null || _b === void 0 ? void 0 : _b.primaryEmotions) || []
            });
        });
    }
    ...
    async getUserSettings(userId) {
        ...
        return ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.settings) || {};
    }
    analyzeCircadianRhythm(journalData, voiceData, engagementData) {
    ...
    async identifyEnergyCycles(journalData, voiceData) {
    ...
    calculateInterventionWindows(circadianRhythm, energyCycles) {
        return {
    ...
    async assessEmotionalReceptivity(journalData) {
        ...
        const hourlyReceptivity = {};
        receptivityData.forEach(({ hour, receptivity }) => {
            if (!hourlyReceptivity[hour])
                hourlyReceptivity[hour] = [];
            hourlyReceptivity[hour].push(receptivity);
        });
        const avgHourlyReceptivity = {};
    ...
    calculateProfileAccuracy(journalData, voiceData, engagementData) {
        let accuracy = 0.5; // Base accuracy
        // More data points increase accuracy
        if (journalData.length > 30)
            accuracy += 0.2;
        if (voiceData.length > 10)
            accuracy += 0.1;
    ...
    generateDefaultHourlyScores() {
        // Default circadian-based receptivity scores
        const scores = {};
        for (let hour = 0; hour < 24; hour++) {
            if (hour >= 6 && hour <= 9)
                scores[hour.toString()] = 0.8; // Morning high
            else if (hour >= 10 && hour <= 11)
                scores[hour.toString()] = 0.6; // Late morning
            else if (hour >= 12 && hour <= 14)
                scores[hour.toString()] = 0.4; // Midday dip
            else if (hour >= 15 && hour <= 17)
                scores[hour.toString()] = 0.7; // Afternoon recovery
            else if (hour >= 18 && hour <= 20)
                scores[hour.toString()] = 0.8; // Evening peak
            else if (hour >= 21 && hour <= 22)
                scores[hour.toString()] = 0.5; // Wind down
            else
                scores[hour.toString()] = 0.2; // Late night/early morning
        }
        return scores;
    }
    generateDefaultWeekdayFactors() {
        return {
    ...
    createFallbackReceptivityModel(userId) {
        return {
            userId,
    ...
    findOptimalTimesForType(interventionType, userProfile, receptivityModel) {
        const optimalTimes = [];
        ...
        const combinedOptimal = peakHours.filter(hour => highReceptivityHours.includes(hour.toString()));
    ...
    calculateStressStateAdjustments(userProfile) {
        return {
            'high_stress': {
    ...
    calculateEmotionalStateAdjustments(receptivityModel) {
        return {
            'positive_state': {
                boost: 1.2,
    ...
    calculateContextualAdjustments(userProfile) {
        return {
    ...
    shouldScheduleIntervention(type, day, config) {
        switch (type) {
            case 'daily_check_in':
    ...
    findBestTimeForDate(date, optimalTimes, userProfile) {
    ...
    async generatePersonalizedContent(type, userId, config) {
    ...
    calculateInterventionIntensity(type, config, day) {
        ...
        if (type === 'crisis_prevention')
            return 'high';
        if (type === 'daily_check_in')
            return 'medium';
        return baseIntensity;
    }
    predictEngagement(type, scheduledTime, userProfile) {
    ...
    identifyPersonalizationFactors(userProfile) {
        const factors = ['circadian_rhythm', 'engagement_patterns'];
        if (userProfile.profileAccuracy > 0.8)
            factors.push('high_confidence_timing');
        if (userProfile.personalPreferences.preferredContactTimes.length > 0)
            factors.push('user_specified_preferences');
        return factors;
    }
    // Helper methods for circadian analysis
    findMidDayDip(hourlyActivity) {
    ...
    inferEnergyFromEmotions(emotions) {
    ...
    identifyHighEnergyPeriods(morning, afternoon, evening) {
        const periods = [];
        if (morning > 60)
    ...
    identifyLowEnergyPeriods(morning, afternoon, evening) {
        const periods = [];
        if (morning < 40)
            periods.push('morning');
        if (afternoon < 40)
    ...
    identifyEnergyTransitions(energyPatterns) {
        // Simple transition detection
        return ['morning_to_afternoon', 'afternoon_to_evening'];
    }
    calculateWeekendDifferences(journalData, voiceData) {
    ...
    combineOptimalTimes(peakHours, energyPeriods) {
        // Combine circadian peaks with energy periods
        return peakHours.map(h => h.toString());
    }
    identifyAcceptableTimes(circadianRhythm, energyCycles) {
        // Times that are acceptable but not optimal
        return ['11', '14', '16', '18'];
    }
    calculateEmotionalReceptivity(emotions) {
    ...
    identifyReceptivityPatterns(hourlyReceptivity) {
    ...
    calculatePatternConsistency(journalData) {
...
exports.PersonalizedInterventionService = PersonalizedInterventionService;
//# sourceMappingURL=personalizedInterventionService.js.map