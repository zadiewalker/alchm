...
exports.updateShardMetricsOnActivity = exports.getUserShard = exports.assignUserShard = exports.monitorShardPerformance = exports.optimizeUserSharding = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const sharding_strategy_1 = require("../../src/lib/database/sharding-strategy");
...
exports.optimizeUserSharding = functions.pubsub
    ...
    try {
        ...
        await sharding_strategy_1.ShardMigrationService.analyzeAndMigrate();
...
exports.monitorShardPerformance = functions.pubsub
...
exports.assignUserShard = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const shardId = await sharding_strategy_1.IntelligentSharding.getOptimalShard(userProfile);
...
exports.getUserShard = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { userId } = data;
    try {
        const shardId = await sharding_strategy_1.ShardRouter.getShardForUser(userId);
...
exports.updateShardMetricsOnActivity = functions.firestore
...
async function calculateLatencyP95(shardId) {
...
async function calculateErrorRate(shardId) {
...
async function generateOptimizationReport(activeUserCount) {
    const report = {
        timestamp: new Date(),
...
async function getSystemHealthScore() {
    ...
    const healthyShards = metrics.filter(m => m.latencyP95 < 200 &&
        m.errorRate < 0.005 &&
        m.capacity < 0.8);
...
async function sendOptimizationAlert(status, message) {
