...
exports.aiModerationCheck = aiModerationCheck;
exports.calculateMatchScore = calculateMatchScore;
exports.applyTraumaInformedFilters = applyTraumaInformedFilters;
exports.calculateCommunityHealthScore = calculateCommunityHealthScore;
exports.escalateCrisisAlert = escalateCrisisAlert;
exports.validateAnonymousId = validateAnonymousId;
...
function generateAnonymousId(userId, context) {
...
async function aiModerationCheck(content, contentType) {
    try {
...
async function basicModerationCheck(content) {
...
function getStageCompatibility(stage1, stage2) {
    ...
    if (stage1 === 'wisdom_sharing' || stage2 === 'wisdom_sharing')
        return 0.9; // Mentorship potential
    if (difference === 2)
        return 0.5;
    return 0.2;
}
function getSupportCompatibility(type1, type2) {
    if (type1 === 'mutual_support' && type2 === 'mutual_support')
        return 1;
    if ((type1 === 'seeking_support' && type2 === 'offering_support') ||
        (type1 === 'offering_support' && type2 === 'seeking_support'))
        return 0.9;
    if (type1 === type2)
        return 0.7;
    return 0.3;
}
function getTimeZoneCompatibility(tz1, tz2) {
...
// === CONTENT FILTERING ===
function applyTraumaInformedFilters(content) {
    const triggerWords = [
...
async function calculateCommunityHealthScore() {
    try {
        const now = new Date();
        ...
        if (moderationRate > 0.1)
            healthScore -= 20;
        else if (moderationRate > 0.05)
            healthScore -= 10;
...
// === CRISIS ESCALATION ===
async function escalateCrisisAlert(userId, contentId, severity) {
    try {
...
// === RATE LIMITING ===
async function checkRateLimit(userId, action, windowMinutes = 60, maxActions = 10) {
    try {
        const now = new Date();
...
//# sourceMappingURL=communityUtils.js.map