...
exports.getDataRetentionStats = exports.updateDataRetentionPreferences = exports.enforceUserDataRetention = exports.enforceDataRetentionPolicies = void 0;
 ...
 */
exports.enforceDataRetentionPolicies = functions.pubsub.schedule('0 3 * * *').onRun(async () => {
    var _a, _b, _c;
    ...
    try {
        ...
        for (const settingsDoc of privacySettingsSnapshot.docs) {
            ...
            try {
                ...
                if (((_a = settings.dataRetention) === null || _a === void 0 ? void 0 : _a.journalRetentionMonths) > 0) {
                ...
                if (((_b = settings.dataRetention) === null || _b === void 0 ? void 0 : _b.analysisRetentionMonths) > 0) {
                ...
                if ((_c = settings.dataRetention) === null || _c === void 0 ? void 0 : _c.automaticDeletion) {
 ...
 */
async function applyJournalRetentionPolicy(userId, retentionMonths) {
 ...
 */
async function applyAnalysisRetentionPolicy(userId, retentionMonths) {
    const cutoffDate = new Date();
    cutoffDate.setMonth(cutoffDate.getMonth() - retentionMonths);
 ...
 */
async function applyAutomaticDeletionPolicy(userId, settings) {
    try {
 ...
 */
async function createInactivityWarning(userId, lastActivity) {
 ...
 */
async function scheduleAccountForAutomaticDeletion(userId) {
 ...
 */
async function logRetentionActivity(stats) {
 ...
 */
exports.enforceUserDataRetention = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    ...
    try {
        ...
        if (forceRetention || ((_a = settings.dataRetention) === null || _a === void 0 ? void 0 : _a.automaticDeletion)) {
            // Apply journal retention
            if (((_b = settings.dataRetention) === null || _b === void 0 ? void 0 : _b.journalRetentionMonths) > 0) {
                results.journalEntriesDeleted = await applyJournalRetentionPolicy(userId, settings.dataRetention.journalRetentionMonths);
            }
            // Apply analysis retention
            if (((_c = settings.dataRetention) === null || _c === void 0 ? void 0 : _c.analysisRetentionMonths) > 0) {
                results.analysesDeleted = await applyAnalysisRetentionPolicy(userId, settings.dataRetention.analysisRetentionMonths);
            }
 ...
 */
exports.updateDataRetentionPreferences = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
exports.getDataRetentionStats = functions.https.onCall(async (data, context) => {
    if (!context.auth || !await isAdmin(context.auth.uid)) {
        throw new functions.https.HttpsError('permission-denied', 'Admin access required');
    }
    try {
        ...
        const recentLogsSnapshot = await db.collection('dataRetentionLogs')
            ...
            .limit(30)
            .get();
        const recentActivity = recentLogsSnapshot.docs.map(doc => (Object.assign({ date: doc.data().date.toDate() }, doc.data().stats)));
 ...
 */
function calculateMonthlyTrends(recentActivity) {
    const monthlyData = {};
 ...
 */
function calculateRetentionCompliance(recentActivity) {
