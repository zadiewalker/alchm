...
export interface TechSupportScenario {
  scenarioType: string;
  commonUserInputs: string[];
  responseTemplates: ScenarioResponseTemplate[];
  escalationTriggers: string[];
  successIndicators: string[];
}

export interface ScenarioResponseTemplate {
  emotionalContext: 'neutral' | 'frustrated' | 'overwhelmed' | 'anxious' | 'crisis';
  greeting: string;
 ...
 */
export const TECH_SUPPORT_SCENARIOS: Record<string, TechSupportScenario> = {
  LOGIN_ISSUES: {
 ...
 */
export const SUCCESS_CELEBRATION_TEMPLATES = {
  FIRST_TIME_SUCCESS: {
 ...
 */
export class TechSupportResponseEngine {
  ...
  generateContextualResponse(
    userInput: string,
    scenarioType: keyof typeof TECH_SUPPORT_SCENARIOS,
    responsePhase: 'greeting' | 'acknowledgment' | 'guidance' | 'success' | 'escalation'
  ): string {
    const scenario = TECH_SUPPORT_SCENARIOS[scenarioType];
    ...
    const template = scenario.responseTemplates.find(
      t => t.emotionalContext === assessment.emotionalState
    ) || scenario.responseTemplates[0]; // fallback to neutral
  ...
  private generateCrisisResponse(phase: string, crisisLevel: string): string {
    if (crisisLevel === 'high') {
      return CRISIS_RESPONSE_TEMPLATES.IMMEDIATE_CRISIS_TECH_SUPPORT.recognition + ' ' +
             CRISIS_RESPONSE_TEMPLATES.IMMEDIATE_CRISIS_TECH_SUPPORT.immediate_support;
    } else if (crisisLevel === 'moderate') {
      return CRISIS_RESPONSE_TEMPLATES.OVERWHELM_TECH_SUPPORT.validation + ' ' +
             CRISIS_RESPONSE_TEMPLATES.OVERWHELM_TECH_SUPPORT.simplification;
    } else {
      return CRISIS_RESPONSE_TEMPLATES.OVERWHELM_TECH_SUPPORT.stress_acknowledgment;
    }
  }

  generateSuccessCelebration(
    userHistory: string[],
    complexityLevel: 'simple' | 'complex' | 'breakthrough'
  ): string {
    ...
    if (complexityLevel === 'breakthrough' || hasPersistedThroughMultipleIssues) {
      return SUCCESS_CELEBRATION_TEMPLATES.BREAKTHROUGH_SUCCESS.recognition + ' ' +
             SUCCESS_CELEBRATION_TEMPLATES.BREAKTHROUGH_SUCCESS.transformation_acknowledgment;
    } else if (isFirstTime) {
      return SUCCESS_CELEBRATION_TEMPLATES.FIRST_TIME_SUCCESS.recognition + ' ' +
             SUCCESS_CELEBRATION_TEMPLATES.FIRST_TIME_SUCCESS.empowerment;
    } else {
      return SUCCESS_CELEBRATION_TEMPLATES.PERSISTENT_USER_SUCCESS.recognition + ' ' +
             SUCCESS_CELEBRATION_TEMPLATES.PERSISTENT_USER_SUCCESS.pattern_acknowledgment;
    }
  }
}

export default TechSupportResponseEngine;