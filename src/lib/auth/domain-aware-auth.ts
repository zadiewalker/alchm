...
// Sacred timing constants for trauma-informed authentication
const SACRED_TIMING = {
  transitionDuration: 300,    // Sacred 300ms transitions
  breathingRoomDelay: 750,    // Sage Green anchoring delay
  maxAuthWait: 15000,         // Maximum auth wait time (trauma-informed)
  crisisTimeout: 5000,        // Crisis bypass timeout
  loadingStateDelay: 200,     // Minimum loading state display
  errorDisplayDuration: 4000, // Error message display duration
  successCelebration: 1200    // Success state celebration duration
};
 ...
 * Available as an alternative to guest sign-in for users who prefer account-based auth
 */
export async function signInWithApple(): Promise<{
  ...
  try {
    ...
    let userFriendlyError = 'Apple Sign-In encountered an issue. Take your time.';
    
    if (error.code === 'auth/popup-blocked') {
      userFriendlyError = 'Pop-up blocked. Please allow pop-ups and try again.';
    } else if (error.code === 'auth/popup-closed-by-user') {
      userFriendlyError = 'Sign-in cancelled. Try again when ready.';
    } else if (error.code === 'auth/unauthorized-domain') {
      userFriendlyError = 'Apple Sign-In is not available on this domain.';
    } else if (error.code === 'auth/network-request-failed') {
      userFriendlyError = 'Connection issue. We\'ll keep trying when you\'re ready.';
    }
...
/**
 * Privacy-Compliant Guest/Anonymous Sign-In
 * Allows users to access ALCHM without providing personal information
 */
export async function signInAsGuest(): Promise<{
  ...
  try {
    if (!validateAuthDomain()) {
      return {
        user: null,
        error: 'Guest access not available on this domain.',
        requiresAgeVerification: false
      };
    }
    ...
    // Guest users still need age verification for COPPA compliance
    const requiresAgeVerification = await checkAgeVerificationRequired(result.user);
    ...
    let userFriendlyError = 'Guest access encountered an issue. Take your time.';
    
    if (error.code === 'auth/operation-not-allowed') {
      userFriendlyError = 'Anonymous sign-in is not enabled. Please try email/password or Apple sign-in.';
    } else if (error.code === 'auth/network-request-failed') {
      userFriendlyError = 'Connection issue. We\'ll keep trying when you\'re ready.';
    }
