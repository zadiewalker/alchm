...
exports.ALCHMKnowledgeBase = exports.ESCALATION_CRITERIA = exports.DEVICE_INSTRUCTIONS = exports.SOLUTION_TEMPLATES = exports.ALCHM_KNOWN_ISSUES = void 0;
// Known ALCHM-specific issues with detailed solutions
exports.ALCHM_KNOWN_ISSUES = {
 ...
 */
class ALCHMKnowledgeBase {
    /**
     * Find known issues matching symptoms
     */
    findMatchingIssues(symptoms, category) {
        const matches = [];
        for (const issue of Object.values(exports.ALCHM_KNOWN_ISSUES)) {
     ...
     */
    getSolutions(issueId, deviceContext) {
        const issue = exports.ALCHM_KNOWN_ISSUES[issueId];
        if (!issue)
            return [];
     ...
     */
    getDeviceInstructions(action, userAgent, platform) {
        const instructions = exports.DEVICE_INSTRUCTIONS[action];
        if (!instructions)
            return 'Follow standard browser instructions';
        // Detect specific browser and platform
        if (platform === 'mobile') {
            if (userAgent.includes('Chrome'))
                return instructions.chrome_mobile;
            if (userAgent.includes('Safari'))
                return instructions.safari_mobile;
            if (userAgent.includes('Firefox'))
                return instructions.firefox_mobile;
            return instructions.mobile || 'Use your mobile browser\'s settings menu';
        }
        else {
            if (userAgent.includes('Chrome'))
                return instructions.chrome_desktop;
            if (userAgent.includes('Safari'))
                return instructions.safari_desktop;
            if (userAgent.includes('Firefox'))
                return instructions.firefox_desktop;
     ...
     */
    requiresEscalation(issueId) {
        if (exports.ESCALATION_CRITERIA.immediate.includes(issueId))
            return 'immediate';
        if (exports.ESCALATION_CRITERIA.high_priority.includes(issueId))
            return 'high_priority';
        if (exports.ESCALATION_CRITERIA.standard.includes(issueId))
            return 'standard';
        return 'none';
    }
    /**
     ...
     */
    getTraumaInformedNotes(issueId) {
        const issue = exports.ALCHM_KNOWN_ISSUES[issueId];
    ...
    getRelatedIssues(issueId) {
        ...
        return issue.relatedIssues
            .map(id => exports.ALCHM_KNOWN_ISSUES[id])
            .filter(Boolean);
    }
}
exports.ALCHMKnowledgeBase = ALCHMKnowledgeBase;
exports.default = ALCHMKnowledgeBase;
