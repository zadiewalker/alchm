...
export interface TechSupportPersonality {
  greeting: TechGreeting;
  problemAcknowledgment: ProblemAcknowledgment;
...
export interface ProblemAcknowledgment {
  validation: string[];
  empathy: string[];
  partnership: string[];
  normalizing: string[];
...
export interface EmotionalIntelligence {
  ...
  responseStrategies: EmotionalResponseStrategy[];
}
 ...
 */
export const ALCHM_TECH_SUPPORT_PERSONALITY: TechSupportPersonality = {
  greeting: {
    standard: [
 ...
 */
export const TRAUMA_INFORMED_LANGUAGE_PATTERNS = {
 ...
 */
export const CRISIS_AWARE_TECH_SUPPORT = {
 ...
 */
export class TraumaInformedTechSupport {
  private currentEmotionalState: string = 'neutral';
   ...
   */
  assessEmotionalState(userInput: string): {
    emotionalState: string;
    crisisRisk: 'none' | 'low' | 'moderate' | 'high';
    responseStrategy: EmotionalResponseStrategy | null;
  } {
    ...
    if (ALCHM_TECH_SUPPORT_PERSONALITY.emotionalIntelligence.frustrationSignals.some(signal => 
        input.includes(signal.toLowerCase()))) {
      emotionalState = 'frustration';
    } else if (ALCHM_TECH_SUPPORT_PERSONALITY.emotionalIntelligence.overwhelmSignals.some(signal => 
        input.includes(signal.toLowerCase()))) {
      emotionalState = 'overwhelm';
    } else if (ALCHM_TECH_SUPPORT_PERSONALITY.emotionalIntelligence.anxietySignals.some(signal => 
        input.includes(signal.toLowerCase()))) {
      emotionalState = 'anxiety';
    }

    const responseStrategy = ALCHM_TECH_SUPPORT_PERSONALITY.emotionalIntelligence.responseStrategies
   ...
   */
  private assessCrisisRisk(input: string): 'none' | 'low' | 'moderate' | 'high' {
    const hopelessnessCount = CRISIS_AWARE_TECH_SUPPORT.crisisIndicators.hopelessness
      .filter(indicator => input.includes(indicator)).length;
    const isolationCount = CRISIS_AWARE_TECH_SUPPORT.crisisIndicators.isolation
      .filter(indicator => input.includes(indicator)).length;
    const overwhelmCount = CRISIS_AWARE_TECH_SUPPORT.crisisIndicators.overwhelm
      .filter(indicator => input.includes(indicator)).length;

    const totalIndicators = hopelessnessCount + isolationCount + overwhelmCount;

    if (totalIndicators >= 3) return 'high';
    if (totalIndicators >= 2) return 'moderate';
    if (totalIndicators >= 1) return 'low';
    return 'none';
  }
  ...
  generateResponse(
    ...
    responseType: 'greeting' | 'acknowledgment' | 'guidance' | 'success' | 'escalation'
  ): string {
   ...
   */
  private generateCrisisAwareResponse(crisisRisk: string, responseType: string): string {
    const acknowledge = CRISIS_AWARE_TECH_SUPPORT.crisisAwareResponses.acknowledge[0];
    const redirect = CRISIS_AWARE_TECH_SUPPORT.crisisAwareResponses.redirect[0];
    const maintain = CRISIS_AWARE_TECH_SUPPORT.crisisAwareResponses.maintain_role[0];
   ...
   */
  private generateEmotionallyTunedResponse(
    assessment: { emotionalState: string; responseStrategy: EmotionalResponseStrategy | null },
    responseType: string
  ): string {
   ...
   */
  transformLanguage(technicalText: string): string {
    let transformedText = technicalText;
    
    // Apply error transformations
    Object.entries(TRAUMA_INFORMED_LANGUAGE_PATTERNS.errorTransformations).forEach(([original, transformed]) => {
      transformedText = transformedText.replace(new RegExp(original, 'gi'), transformed);
    });
    
    // Apply instruction transformations
    Object.entries(TRAUMA_INFORMED_LANGUAGE_PATTERNS.instructionTransformations).forEach(([original, transformed]) => {
      transformedText = transformedText.replace(new RegExp(original, 'gi'), transformed);
    });
    
    // Apply success transformations
    Object.entries(TRAUMA_INFORMED_LANGUAGE_PATTERNS.successTransformations).forEach(([original, transformed]) => {
      transformedText = transformedText.replace(new RegExp(original, 'gi'), transformed);
    });
...
export default ALCHM_TECH_SUPPORT_PERSONALITY;