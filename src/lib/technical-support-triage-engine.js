...
exports.TechnicalSupportTriageEngine = exports.ALCHM_ISSUE_CATEGORIES = void 0;
...
exports.ALCHM_ISSUE_CATEGORIES = {
...
class TechnicalSupportTriageEngine {
    constructor() {
     ...
     */
    async analyzeUserIssue(userInput, deviceContext, userHistory) {
        ...
        const solutions = await this.generateSolutions(categoryResult.category, categoryResult.subcategory, fullDeviceContext, urgencyAssessment.level);
        // Step 7: Determine if escalation is needed
        const escalationRequired = this.shouldEscalate(urgencyAssessment, emotionalAssessment, crisisDetected, solutions.length === 0);
        // Step 8: Select appropriate support personality
        const supportPersonality = this.selectSupportPersonality(emotionalAssessment.emotionalState, urgencyAssessment.level, crisisDetected);
        return {
    ...
    categorizeIssue(userInput) {
        ...
        let bestMatch = {
            category: exports.ALCHM_ISSUE_CATEGORIES.FEATURE_CONFUSION,
            subcategory: 'General Help',
            confidence: 0.3
        };
        for (const [key, category] of Object.entries(exports.ALCHM_ISSUE_CATEGORIES)) {
            let matchScore = 0;
     ...
     */
    determineSubcategory(input, category) {
     ...
     */
    assessUrgency(userInput, category) {
        const input = userInput.toLowerCase();
        let urgencyScore = 0;
        const factors = [];
     ...
     */
    async detectDeviceContext(provided) {
        ...
        const isLowEndDevice = platform === 'mobile' && (screenSize.width < 768 ||
     ...
     */
    async generateSolutions(category, subcategory, deviceContext, urgencyLevel) {
     ...
     */
    getLoginSolutions(subcategory, deviceContext) {
        const baseSteps = [
            {
                order: 1,
     ...
     */
    getJournalSolutions(subcategory, deviceContext) {
        const solutions = [];
     ...
     */
    getMobileSolutions(subcategory, deviceContext) {
        const solutions = [];
     ...
     */
    getBillingSolutions(subcategory, deviceContext) {
        return [{
                id: 'billing_review',
     ...
     */
    shouldEscalate(urgency, emotional, crisis, noSolutions) {
        return (crisis ||
     ...
     */
    selectSupportPersonality(emotionalState, urgencyLevel, crisisDetected) {
        if (crisisDetected)
            return 'crisis-aware';
        if (urgencyLevel === 'critical')
            return 'direct';
     ...
     */
    async generateSupportResponse(triageResult, userInput) {
        const category = triageResult.category;
        const personality = this.selectResponseType(triageResult);
        const response = this.responseEngine.generateContextualResponse(userInput, category.id, 'greeting');
        ...
        return {
            greeting: response,
            problemAcknowledgment: this.responseEngine.generateContextualResponse(userInput, category.id, 'acknowledgment'),
            solutionPlan,
            nextSteps,
            escalationInfo,
            emergencyResources
    ...
    selectResponseType(triageResult) {
        if (triageResult.crisisDetected)
            return 'crisis';
        if (triageResult.emotionalState === 'overwhelm')
            return 'overwhelm';
        if (triageResult.urgency.level === 'critical')
            return 'urgent';
        return 'standard';
    }
}
exports.TechnicalSupportTriageEngine = TechnicalSupportTriageEngine;
exports.default = TechnicalSupportTriageEngine;
