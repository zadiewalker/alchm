...
exports.TraumaInformedTechSupportEngine = void 0;
const technical_support_triage_engine_1 = __importDefault(require("./technical-support-triage-engine"));
const khepera_tech_support_personality_1 = require("./khepera-tech-support-personality");
const khepera_tech_support_response_templates_1 = require("./khepera-tech-support-response-templates");
const alchm_support_knowledge_base_1 = __importDefault(require("./alchm-support-knowledge-base"));
/**
 * Main integration class that combines technical triage with therapeutic presence
 */
class TraumaInformedTechSupportEngine {
    constructor() {
        this.triageEngine = new technical_support_triage_engine_1.default();
        this.traumaSupport = new khepera_tech_support_personality_1.TraumaInformedTechSupport();
        this.responseEngine = new khepera_tech_support_response_templates_1.TechSupportResponseEngine();
        this.knowledgeBase = new alchm_support_knowledge_base_1.default();
        this.activeSessions = new Map();
    }
    /**
     * Start a new therapeutic technical support session
     */
    async startSupportSession(userInput, userId, deviceContext) {
        const sessionId = this.generateSessionId();
        const startTime = new Date();
        // Step 1: Perform technical triage
        const triageResult = await this.triageEngine.analyzeUserIssue(userInput, deviceContext, userId ? this.getUserHistory(userId) : undefined);
        ...
        const knownIssues = this.knowledgeBase.findMatchingIssues([userInput], triageResult.category.id);
        // Step 4: Generate therapeutic response
        const therapeuticResponse = await this.generateTherapeuticResponse(triageResult, emotionalAssessment, userInput, knownIssues);
        // Step 5: Check for intervention needs
        const interventionHistory = this.checkForInterventions(emotionalAssessment, triageResult, userInput);
        const session = {
            sessionId,
            userId,
            startTime,
     ...
     */
    async generateTherapeuticResponse(triageResult, emotionalAssessment, userInput, knownIssues) {
        // Determine personality mode based on emotional state and crisis level
        const personalityMode = this.selectPersonalityMode(emotionalAssessment.emotionalState, emotionalAssessment.crisisRisk, triageResult.urgency.level);
        // Generate therapeutic greeting
        const greeting = this.generateTherapeuticGreeting(personalityMode, emotionalAssessment.emotionalState, triageResult.category.name);
        // Validate the problem with trauma-informed language
        const problemValidation = this.generateProblemValidation(triageResult.category, emotionalAssessment.emotionalState);
        // Acknowledge emotional state
        const emotionalAcknowledgment = this.generateEmotionalAcknowledgment(emotionalAssessment.emotionalState, triageResult.urgency.level);
        // Create technical guidance with therapeutic framing
        const technicalGuidance = this.createTherapeuticTechnicalGuidance(triageResult, emotionalAssessment, knownIssues[0] // Primary matching issue
        );
        // Generate encouragement phrases
        const encouragement = this.generateEncouragement(personalityMode, triageResult.category.id, emotionalAssessment.emotionalState);
        // Define next steps with therapeutic pacing
        const nextSteps = this.generateTherapeuticNextSteps(technicalGuidance, emotionalAssessment.emotionalState);
     ...
     */
    selectPersonalityMode(emotionalState, crisisRisk, urgencyLevel) {
        if (crisisRisk !== 'none')
            return 'crisis-aware';
        if (emotionalState === 'overwhelm' || emotionalState === 'anxiety')
            return 'nurturing';
        if (emotionalState === 'frustration' && urgencyLevel === 'high')
            return 'empowering';
        return 'sage';
    }
    /**
     ...
     */
    generateTherapeuticGreeting(personalityMode, emotionalState, categoryName) {
        const personality = khepera_tech_support_personality_1.ALCHM_TECH_SUPPORT_PERSONALITY;
     ...
     * Validate the problem with trauma-informed language
     */
    generateProblemValidation(category, emotionalState) {
        const baseValidation = khepera_tech_support_personality_1.ALCHM_TECH_SUPPORT_PERSONALITY.problemAcknowledgment.validation[0];
     ...
     */
    generateEmotionalAcknowledgment(emotionalState, urgencyLevel) {
        ...
        return acknowledgments[emotionalState] || acknowledgments.neutral;
    }
    /**
     * Create technical guidance wrapped in therapeutic care
     */
    createTherapeuticTechnicalGuidance(triageResult, emotionalAssessment, primaryIssue) {
        // Determine approach based on emotional state
        let approach = 'confident-direction';
        if (emotionalAssessment.crisisRisk !== 'none')
            approach = 'emergency-focus';
        else if (emotionalAssessment.emotionalState === 'overwhelm')
            approach = 'gentle-exploration';
        // Set pacing based on emotional state and urgency
        let pacing = 'standard';
        if (approach === 'emergency-focus')
            pacing = 'urgent';
        else if (emotionalAssessment.emotionalState === 'overwhelm')
            pacing = 'very-slow';
        else if (emotionalAssessment.emotionalState === 'anxiety')
            pacing = 'patient';
        // Determine complexity level
        const complexityLevel = this.determineComplexityLevel(triageResult.deviceContext, emotionalAssessment.emotionalState, triageResult.urgency.level);
        // Convert solutions to therapeutic steps
        const traumaInformedSteps = this.createTherapeuticSteps(triageResult.recommendedSolutions[0], approach, pacing);
        // Generate safety assurances
        const safetyAssurances = this.generateSafetyAssurances(triageResult.category.id, emotionalAssessment.emotionalState);
        // Create empowerment elements
        const empowermentElements = this.generateEmpowermentElements(triageResult.category.id, approach);
     ...
     */
    createTherapeuticSteps(solution, approach, pacing) {
        if (!solution?.steps)
            return [];
     ...
     */
    transformToTherapeutic(instruction, approach) {
        ...
        const patterns = transformations[approach] || transformations['confident-direction'];
     ...
     */
    generateEmotionalPreparation(step, pacing) {
     ...
     */
    generateStepEncouragement(stepNumber) {
     ...
     */
    generateCelebrationMoment(stepNumber, totalSteps) {
     ...
     */
    determineComplexityLevel(deviceContext, emotionalState, urgencyLevel) {
        if (emotionalState === 'overwhelm')
            return 'simplified';
        if (deviceContext.isLowEndDevice)
            return 'simplified';
     ...
     */
    generateSafetyAssurances(categoryId, emotionalState) {
        ...
        return [
            ...baseAssurances,
            ...(categorySpecific[categoryId] || [])
        ];
    }
    /**
     * Generate empowerment elements
     */
    generateEmpowermentElements(categoryId, approach) {
        const base = [
     ...
     */
    generateBackupPlans(triageResult, primaryIssue) {
        const plans = [
     ...
     */
    generateEncouragement(personalityMode, categoryId, emotionalState) {
        ...
        return [
            ...baseEncouragement,
            ...(modeSpecific[personalityMode] || [])
        ];
    }
    /**
     * Generate therapeutic next steps
     */
    generateTherapeuticNextSteps(technicalGuidance, emotionalState) {
     ...
     */
    generateCrisisResources(crisisLevel) {
        const resources = [
            {
                type: 'phone',
     ...
     */
    checkForInterventions(emotionalAssessment, triageResult, userInput) {
        const interventions = [];
     ...
     */
    determineEscalationPath(triageResult, emotionalAssessment) {
        if (emotionalAssessment.crisisRisk !== 'none') {
     ...
     */
    shouldScheduleFollowUp(emotionalAssessment, triageResult) {
        return (emotionalAssessment.crisisRisk !== 'none' ||
     ...
     */
    async updateSession(sessionId, userResponse, satisfactionLevel) {
        ...
        const newInterventions = this.checkForInterventions(newEmotionalAssessment, session.triageResult, userResponse);
    ...
    completeSession(sessionId, resolution) {
        ...
        if (resolution.resolved && session.knownIssues.length > 0) {
            resolution.preventionEducation = this.knowledgeBase.getPreventionTips(session.knownIssues[0].id);
        }
        return session;
    }
    /**
...
exports.TraumaInformedTechSupportEngine = TraumaInformedTechSupportEngine;
exports.default = TraumaInformedTechSupportEngine;
