import type {
  RelevantReturnCandidate,
  ReturnCandidateMetadata,
  ReturnHistoryMetadata,
  ReturnType,
} from '@/types/return';

interface SelectRelevantCandidatesInput {
...
function getTonePairingFit(currentTone: string, candidateTone: string): number {
...
function inferReturnType(currentEntry: ReturnCandidateMetadata, candidate: ReturnCandidateMetadata): ReturnType {
  ...
  if (getTonePairingFit(currentEntry.emotionalTone, candidate.emotionalTone) >= 2) {
    return 'contrast';
  }
...
export function selectRelevantCandidates({
  ...
  candidates,
  recentReturns,
}: SelectRelevantCandidatesInput): RelevantReturnCandidate[] {
  ...
  return candidates
    ...
    .map((candidate) => {
      ...
      const tonePairingFit = getTonePairingFit(currentEntry.emotionalTone, candidate.emotionalTone);
      const ageDays = Math.max(0, Math.floor((currentEntry.createdAt - candidate.createdAt) / (1000 * 60 * 60 * 24)));
      const ageRelevance = Math.min(ageDays / 14, 3);
      const relevance = (sharedThemes * 5) + recurrenceWeight + tonePairingFit + ageRelevance - (overexposureCount * 6);
