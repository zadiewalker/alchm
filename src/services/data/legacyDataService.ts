...
class DataService {
  ...
  async getUserProfile(): Promise<UserProfile | null> {
    ...
    try {
      const userDoc = doc(getFirestoreDb(), 'users', this.userId);
  ...
  async saveJournalEntry(entry: Omit<JournalEntry, 'id' | 'userId'>): Promise<string> {
    ...
    try {
      const entriesRef = collection(getFirestoreDb(), 'journalEntries');
  ...
  async savePathwayProgress(progress: Omit<PathwayProgress, 'id' | 'userId'>): Promise<void> {
    ...
    try {
      const progressRef = doc(getFirestoreDb(), 'pathwayProgress', `${this.userId}_${progress.pathwayId}`);
  ...
  async getPathwayProgress(pathwayId: string): Promise<PathwayProgress | null> {
    ...
    try {
      const progressRef = doc(getFirestoreDb(), 'pathwayProgress', `${this.userId}_${pathwayId}`);
  ...
  async saveDreamEntry(dream: Omit<DreamEntry, 'id' | 'userId'>): Promise<string> {
    ...
    try {
      const dreamsRef = collection(getFirestoreDb(), 'dreamEntries');
  ...
  async saveShadowWorkProgress(progress: Omit<ShadowWorkProgress, 'id' | 'userId'>): Promise<void> {
    ...
    try {
      const progressRef = doc(getFirestoreDb(), 'shadowWorkProgress', this.userId);
      await setDoc(progressRef, {
        ...progress,
        userId: this.userId,
