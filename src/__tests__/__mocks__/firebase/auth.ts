...
export const signInWithEmailAndPassword = jest.fn((auth: any, email: string, password: string) => {
  const mockUser = {
    uid: 'test-user-id',
    email,
    emailVerified: true,
...
export const createUserWithEmailAndPassword = jest.fn((auth: any, email: string, password: string) => {
  const mockUser = {
...
export const signOut = jest.fn((auth: any) => {
  mockCurrentUser = null
  return Promise.resolve()
})
