...
test('release certification remains evidence-gated until required authorities are proven', () => {
  ...
  assert.match(sameShaLineage, /STALE DEPLOYMENT LINEAGE FOR NEW CANDIDATE/);
  assert.match(sameShaLineage, /ALCHM is not currently same-SHA deployment certified/);
  ...
  assert.equal(runtimeAttestationEvidence.evidence.rollbackAuthority.verified, false);
  ...
  assert.match(deploymentLineageVerification, /STALE FOR NEW CANDIDATE - SAME-SHA DEPLOYMENT EVIDENCE REQUIRED/);
