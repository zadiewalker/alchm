...
test('release certification remains evidence-gated until required authorities are proven', () => {
  ...
  assert.equal(runtimeAttestationEvidence.receipt.payload.authorizationScope, 'continuity-transitions');
  assert.equal(runtimeAttestationEvidence.receipt.payload.candidateSha, runtimeAttestationEvidence.candidateSha);
  assert.equal(runtimeAttestationEvidence.receipt.payload.verifierId, 'alchm-release-owner-2026-05');
