...
test('privacy export and deletion include canonical user journal data', () => {
  ...
  assert.match(dataRightsMap, /continuity\/provenance export coverage is not yet implemented/);
...
test('release certification remains evidence-gated until required authorities are proven', () => {
  ...
  assert.match(runtimeAttestation, /FAIL CLOSED - NO RUNTIME ENABLEMENT AUTHORIZED/);
  assert.match(runtimeAttestation, /environment configuration is an assertion, not attestation evidence/i);
  assert.equal(runtimeAttestationEvidence.attestationStatus, 'NOT ATTESTED');
  assert.equal(runtimeAttestationEvidence.runtimeEnablementAuthorized, false);
  assert.equal(runtimeAttestationEvidence.candidateSha, null);
  assert.equal(runtimeAttestationEvidence.deploymentEnvironment, null);
  assert.equal(runtimeAttestationEvidence.receipt, null);
  assert.equal(trustedRuntimeVerifiers.status, 'NO APPROVED VERIFIER');
  ...
  assert.deepEqual(trustedRuntimeVerifiers.approvedVerifiers, []);
  ...
  assert.match(runtimeAttestationCheck, /verification authority is absent or unverifiable/);
