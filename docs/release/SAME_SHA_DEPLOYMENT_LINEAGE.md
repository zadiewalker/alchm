...
| --- | --- | --- |
| Candidate source | Clean reviewed commit SHA and normalized release scope | New authoritative candidate is `f9c925d707263479953d271eadef69339a5a41bb`; release scope check passes locally |
| CI validation | Green required checks for that identical SHA | Local validation suite passed for `f9c925d707263479953d271eadef69339a5a41bb`; GitHub Navigation E2E run `27169434088` completed successfully for the exact same SHA |
...
| Hosting runtime | Vercel-hosted exported Next artifact; Firebase Hosting redirect edge only | Candidate preview `dpl_5ztQTAGwsag25FMesMM7grG7dPqM` cloned commit `f9c925d`; it was promoted to production deployment `dpl_GbFxz3jiBzFJWZvaKxvwBzKfJnbW`, ready at `https://alchm-ll4ydqqlv-zadie-walkers-projects.vercel.app` with alias `https://alchm.vercel.app`. |
| Native archive | Reconciled app identity, sync and archive evidence attributable to SHA | Same-SHA local archive `/tmp/alchm-return-flow-candidate.xcarchive` succeeded for candidate `f9c925d707263479953d271eadef69339a5a41bb` with RevenueCat native frameworks present. Distribution signing proof, TestFlight upload proof, and RevenueCat entitlement purchase/restore proof remain absent. |
| Rollback | Named prior deploy/archive authority and verified rollback procedure | Current Vercel production deployment is `dpl_GbFxz3jiBzFJWZvaKxvwBzKfJnbW`; previous production rollback target is `dpl_AcCFEEPfBALMwWLsf783dmUf5E9E`; Firebase rollback remains targeted redeploy to project `alchm-463017`. |
...
validation-normalized footer submit navigation candidate
`f9c925d707263479953d271eadef69339a5a41bb`. This candidate was pushed,
validated locally, verified by GitHub Navigation E2E run `27169434088`,
archived locally with RevenueCat native frameworks present, and promoted to
Vercel production deployment `dpl_GbFxz3jiBzFJWZvaKxvwBzKfJnbW`.
Runtime attestation receipt
`PENDING_NEW_RECEIPT_FOR_RETURN_FLOW_CANDIDATE` is recorded for the candidate.
