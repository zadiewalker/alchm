...
| --- | --- | --- |
| Candidate source | Clean reviewed commit SHA and normalized release scope | New authoritative candidate is `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`; release scope check passes locally |
| CI validation | Green required checks for that identical SHA | Local validation suite passed for `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`; GitHub Navigation E2E run `27116669389` completed successfully for the exact same SHA |
...
| Firestore rules | Deployed rules version attributable to SHA and emulator-tested candidate rules | `firestore.rules` is unchanged in candidate `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`, digest is `e142868652eeb0d0f876491cc540357c08f870198d9b2253485e6467a6a982e6`, and direct product-candidate emulator evidence passed with tests=6 pass=6. |
...
| Hosting runtime | Vercel-hosted exported Next artifact; Firebase Hosting redirect edge only | Candidate preview `dpl_Bo3TChAG7xuXXpjDupvFRgoNcffH` cloned commit `23f2eac`; it was promoted to production deployment `dpl_ED8GSN71szSE7TYq6pCLtAiDKA9A`, ready at `https://alchm-5qvfnc30x-zadie-walkers-projects.vercel.app` with alias `https://alchm.vercel.app`. |
| Native archive | Reconciled app identity, sync and archive evidence attributable to SHA | Same-SHA local archive `/tmp/alchm-footer-gating-candidate.xcarchive` succeeded for candidate `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6` with RevenueCat native frameworks present. Distribution signing proof, TestFlight upload proof, and RevenueCat entitlement purchase/restore proof remain absent. |
| Rollback | Named prior deploy/archive authority and verified rollback procedure | Current Vercel production deployment is `dpl_ED8GSN71szSE7TYq6pCLtAiDKA9A`; previous production rollback target is `dpl_ApmU15DALoCc4w9q5WdbBdBkbt98`; Firebase rollback remains targeted redeploy to project `alchm-463017`. |
...
`8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`. This candidate was pushed,
validated locally, verified by GitHub Navigation E2E run `27116669389`,
archived locally with RevenueCat native frameworks present, and promoted to
Vercel production deployment `dpl_ED8GSN71szSE7TYq6pCLtAiDKA9A`.
