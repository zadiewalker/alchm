...
| Local repository validation | Pushed evidence-tail `fe0bf5ee342f1be5177b2a1a843c48c1c8306d3a` validated locally | Passed `npm run design:validate`, `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, `npm --prefix functions run build`, `npm --prefix functions run lint`, `npx cap sync ios`, `npm run check:invariants`, `npm run check:firestore-rules`, `npm run check:integrity`, `npm run check:khepera-gateway`, `npm run check:container-transitions`, `npm run check:sensitive-write-authority`, and `git diff --check`; completed `2026-06-04T16:05:25Z` with clean worktree |
| GitHub checks | Required Navigation E2E workflow ran for pushed evidence-tail `fe0bf5ee342f1be5177b2a1a843c48c1c8306d3a` | Run `26963482032` failed twice; failed step `Run Navigation E2E Suite`; exact failure: timeout waiting for visible `Begin` button in `scripts/e2e-navigation.mjs` `openSplash`; `githubChecksSameSha` remains false |
| Native runtime | Capacitor and generated iOS identifiers are reconciled to `com.alchm.sanctuary` | Local archive succeeded for pushed evidence-tail `fe0bf5ee342f1be5177b2a1a843c48c1c8306d3a`; external RevenueCat dashboard and sandbox/TestFlight entitlement evidence tied to `3a7c6d93140527de77c2fcc91ea3a9f73013b9da` remains required |
