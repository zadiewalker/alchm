...
| Vercel/static export | Vercel production deployment `dpl_ApmU15DALoCc4w9q5WdbBdBkbt98` is `Ready` | Deployment URL `https://alchm-prcrrorrr-zadie-walkers-projects.vercel.app`; aliases include `https://alchm.vercel.app`; production build logs show branch `release/clinical-architecture-integration`, commit `23f2eac`; promoted from ready preview deployment `dpl_G5DnKBuTPHNnADQ4StVorsrSgXzc` |
...
| Local repository validation | Candidate `23f2eac9506c49601403bacf85ffa4196d6734d2` validated locally | Passed `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, `npm --prefix functions run build`, `npm --prefix functions run lint`, `npx cap sync ios`, `npm run check:khepera-gateway`, `npm run check:container-transitions`, `npm run check:sensitive-write-authority`, `npm run check:firestore-emulator-evidence`, and `git diff --check` with clean pre-evidence worktree |
| GitHub checks | Required Navigation E2E workflow ran for candidate `23f2eac9506c49601403bacf85ffa4196d6734d2` | Run `27068348806` completed successfully for exact SHA `23f2eac9506c49601403bacf85ffa4196d6734d2`; run URL `https://github.com/zadiewalker/alchm/actions/runs/27068348806` |
| Native runtime | Capacitor and generated iOS identifiers are reconciled to `com.alchm.sanctuary` | Same-SHA archive `/tmp/alchm-revenuecat-candidate.xcarchive` succeeded for candidate `23f2eac9506c49601403bacf85ffa4196d6734d2`; distribution signing proof, RevenueCat dashboard proof, and TestFlight entitlement proof remain absent |
...
`23f2eac9506c49601403bacf85ffa4196d6734d2`. Final certification remains
