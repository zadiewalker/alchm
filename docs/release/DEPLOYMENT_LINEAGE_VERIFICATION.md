...
| Vercel/static export | Vercel production deployment `dpl_ED8GSN71szSE7TYq6pCLtAiDKA9A` is `Ready` | Deployment URL `https://alchm-5qvfnc30x-zadie-walkers-projects.vercel.app`; aliases include `https://alchm.vercel.app`; production build logs show branch `release/clinical-architecture-integration`, commit `23f2eac`; promoted from ready preview deployment `dpl_Bo3TChAG7xuXXpjDupvFRgoNcffH` |
...
| Local repository validation | Candidate `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6` validated locally | Passed `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, `npm --prefix functions run build`, `npm --prefix functions run lint`, `npx cap sync ios`, `npm run check:khepera-gateway`, `npm run check:container-transitions`, `npm run check:sensitive-write-authority`, `npm run check:firestore-emulator-evidence`, and `git diff --check` with clean pre-evidence worktree |
| GitHub checks | Required Navigation E2E workflow ran for candidate `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6` | Run `27116669389` completed successfully for exact SHA `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`; run URL `https://github.com/zadiewalker/alchm/actions/runs/27116669389` |
| Native runtime | Capacitor and generated iOS identifiers are reconciled to `com.alchm.sanctuary` | Same-SHA archive `/tmp/alchm-footer-gating-candidate.xcarchive` succeeded for candidate `8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`; distribution signing proof, TestFlight upload proof, and RevenueCat entitlement purchase/restore proof remain absent |
...
`prod-23f2eac9506c-2026-06-06T17-22-12-840Z` is recorded for
`8f62cffe7a3f8d5bb0daec3a1e580730bf00fdd6`. Final certification remains
