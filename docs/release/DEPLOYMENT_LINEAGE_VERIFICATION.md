...
| Vercel/static export | Vercel production deployment `dpl_AcCFEEPfBALMwWLsf783dmUf5E9E` is `Ready` | Deployment URL `https://alchm-5v31jsisl-zadie-walkers-projects.vercel.app`; aliases include `https://alchm.vercel.app`; production build logs show branch `release/clinical-architecture-integration`, commit `ba21195`; promoted from ready preview deployment `dpl_H22B7K11pihP2yX35L6AZrBhQAho` |
...
| Local repository validation | Candidate `ba21195ad0b7feff7e3cd8aeb7e04966ad8ccf30` validated locally | Passed `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, `npm --prefix functions run build`, `npm --prefix functions run lint`, `npx cap sync ios`, `npm run check:khepera-gateway`, `npm run check:container-transitions`, `npm run check:sensitive-write-authority`, `npm run check:firestore-emulator-evidence`, and `git diff --check` with clean pre-evidence worktree |
| GitHub checks | Required Navigation E2E workflow ran for candidate `ba21195ad0b7feff7e3cd8aeb7e04966ad8ccf30` | Run `27123588249` completed successfully for exact SHA `ba21195ad0b7feff7e3cd8aeb7e04966ad8ccf30`; run URL `https://github.com/zadiewalker/alchm/actions/runs/27123588249` |
| Native runtime | Capacitor and generated iOS identifiers are reconciled to `com.alchm.sanctuary` | Same-SHA archive `/tmp/alchm-footer-submit-back-candidate.xcarchive` succeeded for candidate `ba21195ad0b7feff7e3cd8aeb7e04966ad8ccf30`; distribution signing proof, TestFlight upload proof, and RevenueCat entitlement purchase/restore proof remain absent |
...
`prod-ba21195ad0b7-2026-06-08T08-23-18-117Z` is recorded for
`ba21195ad0b7feff7e3cd8aeb7e04966ad8ccf30`. Final certification remains
