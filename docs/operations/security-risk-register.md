...
- `uuid` is forced to `^11.1.1` to remove transitive moderate advisory paths through Firebase Admin, Google Cloud clients, and Sentry's webpack plugin.

These overrides must remain covered by:

- `npm ci`
- `npm ls firebase-admin next postcss @tootallnate/once uuid`
- `npm audit --omit=dev --audit-level=moderate`
